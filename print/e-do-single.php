...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$nomor_do = $_GET['nomor_do'];
		$this->writeHTMLCell(0, 0, '', '', $htmlz, '', 1, 0, true, 'L', true);
		$this->SetFont('helvetica', '', 4);
		$this->writeHTMLCell(0, 0, '', '', '<p></p>', '', 1, 0, true, 'R', true);
		$this->SetFont('helvetica', '', 12);
		$this->writeHTMLCell(0, 0, '', '',$nomor_do, '', 1, 0, true, 'R', true);
		$this->SetFont('helvetica', '', 9);
		$this->writeHTMLCell(0, 0, '', '', '<h2 style="border-bottom:3px solid black"></h2>', '', 1, 0, true, 'L', true);
	}
	...
	$today = strtoupper(date("d F Y")); 
	$htmlza .= 'JAKARTA, ' . $today;
	$htmlza .= '<br><br><br><br><br><br>						    
...
$htmlatasz = '
<br>
<h2 style="padding-bottom:5px">DELIVERY ORDER<hr style="width:155px"/><b style="font-size:9.5px">SURAT PERINTAH PENYERAHAN</b></h2>

';

$pdf->writeHTMLCell(0, 0, '', '', $htmlatasz, '', 1, 0, true, 'C', true);
include '../config/configurasi.php';
$datado = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * from delivery_order where nomor_do='$_GET[nomor_do]'"));
$html = '
<br>
<table cellpadding="6" cellspacing="0" border="1" style="font-size:10px">
	...
	<tr>
		...
		$pelabuhan_asal	=	substr($datado['pelabuhan_asal'], 0,2);
		$tgl_masuk	=	date("d-m-Y",strtotime($datado['tgl_masuk']));
		$datapelabulan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT name from country where iso='$pelabuhan_asal'"));
		$pelabuhan_asal = $datapelabulan['name'];
	...
	<tr>
		...
		$pelabuhan_akhir	=	substr($datado['pelabuhan_akhir'], 0,2);
		$tgl_tiba	=	date("d-m-Y",strtotime($datado['tgl_tiba']));
		$datapelabulan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT name from country where iso='$pelabuhan_akhir'"));
		$pelabuhan_akhir = $datapelabulan['name'];
